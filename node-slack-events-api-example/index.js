...
const botAuthorizations = {
  T93LP16LV: 'xoxb-316324228368-sKzUWWQIZaRVuGoeXDyIrDWX'
}
...
slackEvents.on('message', (message, body) => {
  ...
  // console.log(message, body.team_id)
  if (!message.subtype && message.text.indexOf('hi') >= 0) {
    ...
    axios
      .get('http://lp.edu.ua/rozklad-dlya-vykladachiv')
      .then(resp => {
        // console.log(resp.data)
        // Respond to the message back in the same channel
        slack.chat.postMessage(message.channel, `Hello <@${message.user}>! :tada:`)
        // slack.chat.postMessage(message.channel, resp.data)
          .catch(console.error);
      })
  }
});
